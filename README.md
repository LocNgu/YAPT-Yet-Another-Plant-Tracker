...
- **Check-in reminders** — A watering-due reminder is titled "Check {plant}" with two actions: **Watered**, and **Not now** (pushes the due date back a day)
...
- **Dormancy window** — Give a plant the months it rests (e.g. Nov–Feb, wrapping the new year is fine) on Add/Edit Plant or the Water tab's settings. While dormant, watering and fertilizing reminders pause, the plant shows as Dormant on the Plant List and Calendar (without counting as due), and "Why this date?" explains the suspended schedule. Optionally keep watering on a fixed 1–12 week dormant cadence instead of a full pause — it ignores seasonal/adaptive changes and shows up normally in due lists, Calendar, and reminders, while fertilizing stays paused. Waterings that span the rest period don't teach the adaptive model, and the first watering afterwards lowers its confidence by one step
- **Fertilizing active seasons** — Choose which of Spring, Summer, Autumn, and Winter a plant fertilizes in (default: all four), at its one fertilizing interval, on Add/Edit Plant or the Fertilize tab. A plant is never due or overdue for fertilizing during an inactive season and becomes due on the first day it re-enters an active one; seasons follow your device's timezone/hemisphere
...
- **Interval steppers** — Every interval slider (watering, fertilizing, repotting, dormant watering cadence) has −/+ buttons for exact one-step nudges plus a light haptic tick while dragging; on Plant Detail's watering and fertilizing cards, a quick burst of taps saves as a single change
...
- **Watering actions with off-schedule reasons** — Plant Detail's **Water** and **Reschedule watering** buttons are always available. Tapping Water (or its liquid-fertilizer counterpart) opens a "Log watering" date picker pre-selected to today — confirm as-is for an instant log, or pick an earlier date to backfill a watering you forgot to log. Watering off schedule asks a reason worded for the direction it was missed: early asks "Why now?" — "The plant needed it" / "Just my schedule"; late asks "Why was it late?" — "Soil was still moist" / "Forgot, or no time" (a late watering can only hold or lengthen the interval, never shorten it). Reschedule watering opens a date picker directly with no reason prompt — Today / +1 / +2 / +3 days / a custom date — and never affects the adaptive watering model; all learning comes from the next actual watering
- **Reschedule delta chip** — When a reschedule is currently pushing a plant's watering due date out, Plant Detail shows a "Rescheduled +N days" chip above the Water/Reschedule row; tap it to revert instantly, with an Undo snackbar. Never affects the adaptive watering model — it's purely about the due date
- **Repot and Photo quick actions** — Plant Detail's Repot and Photo tabs have always-visible quick actions that open a "Log watering"-style date picker (defaulting to today) before logging: Repot anchors both the care log and the repot confidence-reset/freeze window to the picked date, and Add photo opens one sheet with an editable date plus Take photo / Choose from gallery buttons, updating the cover photo directly. The full Add Care Log screen remains available via the + button for adding a photo with notes
...
- **Date-group dividers** — When sorted by Watering due, Fertilizing due, or Both due, the plant list groups cards under Overdue / Today / Tomorrow / dated / Later / Not scheduled headers (plus a Dormant group for plants whose schedule is paused by their dormancy window)
...
- **Photo gallery** — Per-plant gallery of all plant and care-log photos, sorted by date; tap any photo to open a full-screen swipe viewer that shows each photo's date; add photos via the gallery picker or by taking a new photo with the in-app camera; long-press or use the trash icon in the viewer to delete individual photos. In-app camera captures are resized (max 1920 px) and saved as compact JPEGs to save storage
- **Plant Graveyard** — Deleted plants move to an archive in Settings; restore them or remove them permanently
- **Backup & restore** — Export and import a `.yapt` ZIP file via the system file picker, with optional photo inclusion and an optional "Optimize photos" export checkbox (off by default) that compresses the backup's photo copies without touching the originals
- **Developer mode** — Tap the Settings → About version row five times to unlock a hidden Developer section: build info, a feature-flag registry for opt-in experimental features (currently empty), and debug actions (reset the What's New sheet, run the reminder check now, seed/remove demo plants). Off by default and excluded from backup
