...
- **Dormancy window** — Give a plant the months it rests (e.g. Nov–Feb, wrapping the new year is fine) on Add/Edit Plant or the Water tab's settings. While dormant, watering reminders pause, the plant shows as Dormant on the Plant List and Calendar (without counting as due), and "Why this date?" explains the suspended schedule; fertilizing stays on its normal schedule. Waterings that span the rest period don't teach the adaptive model, and the first watering afterwards lowers its confidence by one step
...
- **Date-group dividers** — When sorted by Watering due, Fertilizing due, or Both due, the plant list groups cards under Overdue / Today / Tomorrow / dated / Later / Not scheduled headers (plus Dormant in the Watering due and Both due sorts)
