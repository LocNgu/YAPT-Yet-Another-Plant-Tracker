...
- **Reschedule option dates** (#737) — Today and +1/+2/+3 display their resulting dates via `DateUtils.formatDate()`. The +N preview and handler share `rescheduledRelativeDueAt()` so both anchor to `maxOf(nextWateringDueAt, now)`; the removed "(suggested)" row (#738) is not part of the current dialog. Custom date remains a picker.
