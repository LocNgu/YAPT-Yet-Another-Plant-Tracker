...
- Per-action tabs (#436, Option 2 — sub-task 1 of 3): a Material 3 `PrimaryTabRow` (**Water · Fertilize · Repot · Photo**) added as a `LazyColumn` item **inside the existing `Box` overlay, below the hero + name header** — no `Scaffold`/`TopAppBar`, so the 280 dp edge-to-edge hero survives (see technical **ADR-0018**, which supersedes ADR-0005). `PlantDetailTab` enum holds each tab's `labelRes` + icon; `selectedTab` is `rememberSaveable` (defaults to Water). Content relocated per tab: Water = watering-history chart + Skip-watering button + a "Recent misting" `MIST`-log list (misting folded into Water per the #436 spec); Fertilize/Repot = filtered `FERTILIZE`/`REPOT` care-log lists with per-tab empty states; Photo = the unified `PhotoGallery` with empty state. Per-tab filtered lists use prefixed keys (`"fert-"`/`"repot-"`/`"mist-"` + log id) so they never collide with the shared care-history list's `it.id` keys. The #434 quick-log `StatsRow` chips stay **above** the tab strip (always-visible summary); the unified care-history list (all types incl. Prune/Note) stays **below** the tab content; the `+` FAB / Add Care Log screen are unchanged. Richer per-tab insights are deferred to sub-task 3
- Inline scheduling settings on the tabs (#436, Option 2 — sub-task 2 of 3; see product **ADR-0022**): the Water tab and Fertilize tab each show an editable settings `Card` at the top — Water = watering-interval enable `Switch` + `Slider` (1–60 days); Fertilize = fertilizing-interval `Switch` + `Slider` (1–90 days) + the liquid-fertilizer toggle — mirroring the Add/Edit Plant controls. Edits **auto-persist** (no Save button) via new `PlantDetailViewModel.setWateringInterval(Int?)` / `setFertilizingInterval(Int?)` / `setLiquidFertilizer(Boolean)` → `PlantRepository.updatePlant`; a `null` interval clears the schedule (the "Not scheduled" state), matching the Add/Edit reminder toggle. Slider drags commit on release (`onValueChangeFinished`), not per frame; the shared presentational `InlineIntervalSetting` composable (enable switch + slider + optional `extra` slot for the liquid-fert row) lives in `PlantDetailScreen.kt`; default start intervals when enabling live in `PlantDetailViewModel.DEFAULT_WATERING_INTERVAL_DAYS`/`DEFAULT_FERTILIZING_INTERVAL_DAYS` (7/30). Add/Edit Plant stays the canonical editor for name/species/room/notes/cover photo — only the scheduling fields the tabs visualise are inline-editable (ADR-0022 records this as a *new* product decision, not a supersession)
- Tappable stat chips for in-place quick-logging (#434): the Watering and Fertilizing `StatChip`s in `StatsRow` accept optional `onWaterClick`/`onFertilizeClick` lambdas (with `clickable(onClickLabel = …)` for accessibility — `cd_quick_log_watering`/`cd_quick_log_fertilizing`). On Plant Detail, tapping Watering opens `WaterFeedbackBottomSheet` → `PlantDetailViewModel.quickWater()`; tapping Fertilizing logs FERTILIZE directly via `quickFertilize()` for regular plants, or opens the combined "Water & fertilize" sheet → `quickLiquidFertilize()` for liquid-fertilizer plants (ADR-0008/ADR-0017). All three VM methods delegate persistence to the shared `QuickLogUseCase` (now injected into `PlantDetailViewModel` + `Factory` + `NavGraph`), feed any adaptive suggestion into the existing `suggestedWateringInterval` dialog, and emit a `QuickLogMessage` sealed event that the screen maps to a snackbar; `maybeTriggerPhotoReminder()` reuses the screen's existing photo-reminder dialog (suppressed while the suggestion dialog shows, matching `PlantListScreen`). The `+` FAB and full Add Care Log screen are unchanged for other care types. Option 2 (per-action tabs with inline settings) was deferred — it conflicts with technical ADR-0005, and would establish a *new* inline-settings product decision (the "settings edited only on Add/Edit" rule is a convention, not a recorded ADR — the earlier reference to "product ADR-0012" was inaccurate: product/0012 is *cover-photo source*, already superseded by ADR-0015). Now being delivered in sub-tasks under #436 — see the Plant Detail tabs entry below
...
- "Notify for fertilizing" toggle: Settings → Reminders `Switch` row (gated behind the notifications-enabled toggle, sits between the reminder-time and "Combine reminders" rows), `fertilizing_notifications_enabled` DataStore key, default `true` (existing behaviour unchanged). When **off**, `ReminderNotificationComposer.computeDueReminders(statuses, now, fertilizingNotificationsEnabled = false)` drops any plant whose composed items contain no watering item (a fertilizing-only reminder) via the private `List<CareReminderItem>.hasWateringItem()` helper; a plant that is also watering-due keeps its full body (fertilizing line included) because the watering urgency makes it timely, and liquid-fertilizer plants never produce a fertilizing-only reminder in the first place (`FertilizeWithWatering` is only added alongside a watering item). `ReminderWorker` reads the pref from `context.settingsDataStore` and passes it through; the pure filter is JVM-tested in `ReminderNotificationComposerTest` (both branches) and end-to-end in `ReminderWorkerTest` (fertilizing-only suppressed vs. both-due unchanged). Round-trips through backup as `BackupSettings.fertilizingNotificationsEnabled: Boolean` (backup schema v7; old v6-and-earlier backups deserialize to the default `true` via the field default). See product ADR-0021 (#223)
...
- Backup schema v7: `BackupSettings.fertilizingNotificationsEnabled: Boolean = true` round-trips the "Notify for fertilizing" toggle; old v6-and-earlier backups deserialize to the default (`true`, on) via the field default, so moving devices no longer silently changes it (#223)
