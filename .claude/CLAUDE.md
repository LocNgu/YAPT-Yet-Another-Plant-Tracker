...
- **Robolectric unit tests run against `TestYaptApplication`** — the real application minus `onCreate`'s fire-and-forget `launchAppStartWork()`, registered module-wide by `app/src/test/resources/robolectric.properties`. Robolectric rebuilds the application per *test method* while the Room DB and the DataStore delegate are process-wide singletons, so that coroutine raced test fixtures and rewrote `wateringBaseIntervalDays` (#757). New app-start background work goes inside `launchAppStartWork()` and gets its own direct test; never `@Config(application = YaptApplication::class)`. Details: `.claude/rules/ci-build.md`.
