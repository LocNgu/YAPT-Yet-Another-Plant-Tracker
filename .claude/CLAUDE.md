...
- **Developer mode** (sub-task 1 of 4 for #514; see product ADR-0022): tapping the Settings → About version row 5 times unlocks a **Developer** section at the bottom of Settings, below About/What's New, behind a `HorizontalDivider`. Tap counter logic is a pure, Compose-free `DeveloperModeUnlock.registerTap(currentTapCount, isDeveloperModeEnabled)` in `domain/devmode/` returning a `DeveloperModeTapResult` (new count + `Silent`/`Countdown(tapsRemaining)`/`Unlocked`/`Inert` outcome) — the counter itself is screen-scoped Compose `remember` state in `SettingsScreen`, not the ViewModel, so it resets whenever Settings leaves composition with no wall-clock timeout; once already unlocked every tap is `Inert` (no counter change, no snackbar). AOSP-style countdown snackbars at taps 3/4/5 ("You are 2/1 taps away…" via `dev_mode_countdown_taps_away` plural, then "Developer mode enabled"). `SettingsKeys.DEVELOPER_MODE_ENABLED` DataStore boolean (default false) backs `SettingsViewModel.developerModeEnabled`/`setDeveloperModeEnabled(Boolean)`; the section's first row is a master `Switch` — turning it off hides the section and shows "Developer mode disabled", with a deliberate seam (a comment in `setDeveloperModeEnabled`) for #521 to call `FeatureFlags.resetAll()` there once flags exist (there are none yet). Four read-only, non-interactive build-info rows: version name+code (`BuildConfig`), build type (`BuildConfig.BUILD_TYPE`), Room DB version, device API level (`Build.VERSION.SDK_INT`). The Room DB version is sourced from `PlantDatabase.DB_VERSION`, a single `const val` also referenced by the `@Database(version = ...)` annotation so the two can never drift. Developer mode is reachable in **both debug and release** builds, behaving identically, and is deliberately excluded from backup/restore (device-local, no schema bump). No feature flags or debug actions yet — those are #521/#522/#523 (#520)
