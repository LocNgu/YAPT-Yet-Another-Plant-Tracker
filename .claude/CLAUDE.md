...
- In-app camera capture: `PhotoSourceBottomSheet` lets user choose "Take photo" or "Choose from gallery"; runtime CAMERA permission with rationale dialog on first denial and Settings deep-link on permanent denial; Snackbar error on devices without a camera (#134); shared `rememberCameraPhotoState` + `CameraPhotoDialogs` composable extracted to avoid duplicating ~80-line camera block across AddEditPlantScreen and AddCareLogScreen (#293); on AddCareLog, selecting the PHOTO care type auto-opens `PhotoSourceBottomSheet` via `LaunchedEffect(selectedCareType)` — fires only on the transition into PHOTO (guarded by `photoUri == null` so it never re-opens after a dismiss-while-still-PHOTO, nor auto-opens in edit mode when the PHOTO log already has a photo); the `AddAPhoto` button remains as the re-open path (#443)
