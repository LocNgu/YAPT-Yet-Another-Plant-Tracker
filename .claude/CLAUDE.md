...
- **Integer interval sliders use the shared `SteppedSlider`** (`ui/components/SteppedSlider.kt`) — a `Slider` flanked by −/+ stepper buttons with a haptic tick on drag; commit semantics (immediate vs. commit-on-release) stay per-surface (#531, product ADR-0048). On Plant Detail's inline watering/fertilizing cards, a burst of −/+ taps coalesces into one write + one `MANUAL_EDIT` row after a 1s quiet window (`PlantDetailScheduleSettingsActions.kt`'s `INTERVAL_TAP_COALESCE_WINDOW_MS`) rather than one per tap; both watering/fertilizing writes now read the plant fresh inside `PlantDetailViewModel.intervalEditMutex` instead of the lag-prone cached `plant` StateFlow, and a still-pending tap survives leaving the screen via `PlantDetailViewModel.applicationScope`/`onCleared()` (#531 review round 1, product ADR-0049). The dormancy cadence slider keeps committing every tap immediately (own pre-existing mutex, no audit row).
