...
- Watering history chart on plant detail screen (PR #108, issue #18): bar chart visualization of days between consecutive watering events; time range selector with chips (1M/3M/6M/12M, defaults to 1M); statistics showing last watering date and average interval; Material 3 theme integration using SageGreen for bars and OkGreen for markers; empty state for plants with < 2 watering logs; `computeWateringIntervals()` helper function with 8 unit tests; Vico 2.0.0 M3 Compose dependency added
