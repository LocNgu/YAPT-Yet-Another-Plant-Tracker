...
**Architecture & Data**
- Room DB v7 (PlantEntity, CareLogEntity, PlantPhotoEntity); explicit migrations — hard-crash if migration missing (`fallbackToDestructiveMigration` removed); schemas exported to `app/schemas/`; baseline `1.json` committed; MIGRATION_3_4 creates `plant_photos` table and seeds rows from `coverPhotoUri` (#290); MIGRATION_4_5 adds unique index on (plantId, uri) in plant_photos and deduplicates existing rows (#301); MIGRATION_5_6 adds `archivedAt INTEGER` column to plants table for soft-delete graveyard (#329); MIGRATION_6_7 adds nullable `mistingIntervalDays`/`repottingIntervalDays INTEGER` columns to plants for extended-care reminders (#232)
...
**Domain & Scheduling**
- `CareSchedule.computeStatus()`: also computes **misting** and **repotting** due-status from `plant.mistingIntervalDays`/`repottingIntervalDays` and optional `lastMistedAt`/`lastRepottedAt` params (defaulted null, so existing callers are unaffected); populates 8 defaulted `PlantCareStatus` fields (`next*DueAt`/`is*Overdue`/`is*DueSoon`/`last*At` per type). First-due for a never-done extended care is anchored to `createdAt + interval` (private `extendedCareDueAt()` helper) — a newly added plant isn't flagged immediately (distinct from watering's "due today" and fertilizing's 30-day grace). `ReminderWorker` queries the last `MIST`/`REPOT` log only when the interval is set, so logging that care type resets its due date; `ReminderNotificationComposer` emits `Misting*`/`Repotting*` `CareReminderItem`s joined with `" · "`. Add/Edit Plant has Switch+Slider controls: misting in **days** (1–30, default 7); repotting in **months** (3–36, 1-month steps, default 12) — day precision is false precision for repotting, so the UI state is `repottingIntervalMonths` and is persisted as `months * DAYS_PER_MONTH (30)` into the day-based `Plant.repottingIntervalDays` (read back clamped into 3–36), leaving the schedule logic and DB schema untouched. Round-trips through backup (`BackupPlant.mistingIntervalDays`/`repottingIntervalDays`, schema v8). Extended-care status is intentionally not surfaced on list/detail/calendar yet; free-text custom reminders are a deferred follow-up. See product ADR-0022 (#232)
...
- Backup schema v8: `BackupPlant.mistingIntervalDays`/`repottingIntervalDays: Int? = null` round-trip the extended-care reminder intervals; old v7-and-earlier backups deserialize both to `null` via the field defaults (#232)
