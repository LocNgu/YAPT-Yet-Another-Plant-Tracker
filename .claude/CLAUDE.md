...
- Per-plant liquid fertilizer mode (issue #56): `useLiquidFertilizer: Boolean` on Plant + PlantEntity; `fertilizerType: FertilizerType` (LIQUID/SOLID/UNSPECIFIED) on CareLog + CareLogEntity; Add/Edit Plant screen toggle inside fertilizing-enabled block; Add Care Log screen Liquid/Solid FilterChips pre-selected from plant default; FERTILIZE+LIQUID auto-creates paired WATER log at same timestamp and clears wateringDueDateOverride; quick-fertilize from PlantList also auto-creates paired WATER for liquid plants; PlantCard uses `Icons.Filled.Science` for liquid-fertilizer plants; ReminderWorker shows "Fertilize with watering" on watering notification instead of separate fertilizing line; BackupManager (schema v2) threads useLiquidFertilizer + fertilizerType through export/import; DB version bumped to 3 with MIGRATION_2_3; schema 3.json committed
