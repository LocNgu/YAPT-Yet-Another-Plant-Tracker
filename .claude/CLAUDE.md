...
**Architecture & Data**
- Room DB v7 (PlantEntity, CareLogEntity, PlantPhotoEntity); explicit migrations — hard-crash if migration missing (`fallbackToDestructiveMigration` removed); schemas exported to `app/schemas/`; baseline `1.json` committed; MIGRATION_3_4 creates `plant_photos` table and seeds rows from `coverPhotoUri` (#290); MIGRATION_4_5 adds unique index on (plantId, uri) in plant_photos and deduplicates existing rows (#301); MIGRATION_5_6 adds `archivedAt INTEGER` column to plants table for soft-delete graveyard (#329); MIGRATION_6_7 adds a nullable `repottingIntervalDays INTEGER` column to plants for the repotting reminder (#232)
...
**Domain & Scheduling**
- `CareSchedule.computeStatus()`: also computes **repotting** due-status from `plant.repottingIntervalDays` and an optional `lastRepottedAt` param (defaulted null, so existing callers are unaffected); populates 4 defaulted `PlantCareStatus` fields (`nextRepottingDueAt`/`isRepottingOverdue`/`isRepottingDueSoon`/`lastRepottedAt`). First-due for a never-repotted plant is anchored to `createdAt + interval` (private generic `extendedCareDueAt()` helper) — a newly added plant isn't flagged immediately (distinct from watering's "due today" and fertilizing's 30-day grace). `ReminderWorker` queries the last `REPOT` log only when the interval is set, so logging a repotting resets its due date; `ReminderNotificationComposer` emits `Repotting*` `CareReminderItem`s joined with `" · "`. Add/Edit Plant has a Switch+Slider in **months** (3–36, 1-month steps, default 12) — day precision is false precision for repotting, so the UI state is `repottingIntervalMonths` and is persisted as `months * DAYS_PER_MONTH (30)` into the day-based `Plant.repottingIntervalDays` (read back clamped into 3–36), leaving the schedule logic and DB schema untouched. Round-trips through backup (`BackupPlant.repottingIntervalDays`, schema v8). **Misting was deliberately dropped** before merge (ineffective, fungal-risk practice — `CareType.MIST` stays loggable, it is simply never *scheduled*); repotting status is intentionally not surfaced on list/detail/calendar yet; free-text custom reminders are a deferred follow-up. See product ADR-0022 (#232)
...
- Backup schema v8: `BackupPlant.repottingIntervalDays: Int? = null` round-trips the repotting reminder interval; old v7-and-earlier backups deserialize it to `null` via the field default (#232)
