...
`app/src/test/resources/robolectric.properties` registers `com.yapt.planttracker.TestYaptApplication`
for the whole unit-test module. It is the production application with `launchAppStartWork()` (the
`onCreate` coroutine running `writeDefaultReminderTimeIfAbsent` + `SeasonalGraduationFixup`) overridden
to a no-op — the sole reason `YaptApplication` and that one method are `open`.
...
- Don't add `@Config(application = …)` pointing back at `YaptApplication` — that opts a class back into
  the race. `TestYaptApplicationTest` fails if the properties file stops applying module-wide.
