...
- **Fertilizing** (#795, product ADR-0046, superseding #286's four discrete per-season intervals,
  ...
  the same shift. `SeasonalFertilizing.nextActiveDueAtMillis()` leaves the raw date unchanged when
  its own hemisphere-aware season is in `Plant.fertilizingSeasons`, else moves it to the start of day
  of the 1st of the first following month whose season is active — never due or overdue in an
  inactive season, due exactly on the first day of the next active one on re-entry. Adaptive watering
  never touches this; no learning, confidence, or adjustment history.
