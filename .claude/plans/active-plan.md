...
- [x] Fix #180: `CareSchedule.daysBetween()` uses `ChronoUnit.DAYS.between(toLocalDate(), toLocalDate())` instead of millisecond integer division — eliminates spurious "interval − 1" suggestion when watering on the exact due calendar day with Just Right feedback (PR #182); 2 new `CareScheduleTest` cases
