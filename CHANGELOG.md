...
### Fixed
- **The late-watering "Why was it late?" prompt no longer offers a confusing, mis-shortening "It was dry by then" answer** — that option read as accusatory, and its `TOO_LATE` mapping usually *lengthened* the interval rather than shortening it for a genuinely late gap, extrapolating a precise point estimate from what's really an ambiguous multi-day-overdue window. It's replaced with "Soil was still moist" (`WateringReason.SOIL_STILL_MOIST` → `TOO_SOON`, lengthens), the same signal the Reschedule flow's "Soil still moist" answer already represents, for a user who checked informally rather than tapping Reschedule. A late watering can no longer shorten the interval at all — shortening only ever happens from the early ("Why now?" → "The plant needed it") direction, where the signal is precise. See product ADR-0033 (#649)
