...
### Changed
- **Photos now take substantially less space in YAPT and, when requested, its backups** — new camera captures are resized to at most 1920 px on their longest side and re-encoded as 80% quality JPEGs, and existing in-app camera captures receive the same optimization once after updating. Backup export now offers an optional "Optimize photos" checkbox that compresses the copies placed in the backup—including gallery photos—without changing the originals in the system gallery; it starts unchecked so normal backups retain original fidelity. Orientation and PNG transparency are preserved, unsupported formats remain unchanged, and processing safely falls back to the original image if compression fails (#309, product ADR-0039)
