...
### Fixed
- **History bootstrap now keeps the displayed watering interval in seasonal effective space** — when adaptive watering cold-starts a plant from its watering history, the learned estimate is still stored unchanged as the season-neutral `wateringBaseIntervalDays`, but `wateringIntervalDays` is now written as that base converted for the current seasonal factor. Previously the raw base value was written into this display-facing field, so the Water tab and "Why this date?" sheet could show a stale/wrong "every N days" value until a later adaptation even though the due date itself was already correct. Pinned plants and disabled/Off seasonal watering remain unchanged, and the `HISTORY_BOOTSTRAP` adjustment continues to record base-space model values (#662)
