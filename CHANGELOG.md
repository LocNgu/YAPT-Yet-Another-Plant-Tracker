...
### Fixed
- **Reschedule's "Today" option could be greyed out in a state where it would actually pull the due date in** — it was gated on `PlantCareStatus.isOverdue`, which reflects the *effective* (post-override) due date; since a reschedule override only ever wins when it's later than the schedule-computed date, a plant rescheduled out to a future date could pass its schedule-computed due date and still show "Today" disabled, even though tapping it would legitimately shorten the earlier deferral. The gate now compares local today directly against the schedule-computed due date (`PlantCareStatus.computedNextWateringDueAt`), the same comparison the "Custom date…" picker already makes for its own "today" cell, so the two can no longer disagree; `isOverdue`'s own definition is unchanged. A never-watered plant with a winning future override is a known, accepted, unfixed edge case — its computed due date always tracks "now", so this gate never enables there either, matching the old behavior (#746)
