...
### Fixed
- **The "Water every N days?" suggestion dialog no longer compares a base-space suggestion against an effective-space "current" interval** — with `adaptive_watering` and `seasonal_watering` both on, the dialog's "suggested" number was `QuickLogUseCase`'s adaptive suggestion, computed in season-neutral base space, shown directly alongside `plant.wateringIntervalDays`, which is already seasonally adjusted. Comparing the two in different unit spaces produced misleading multi-day jumps (e.g. "suggested 9, current 7" for a plant only 1 day off schedule) that were almost entirely a unit-mismatch artifact. The suggestion is now converted to effective space via the same `CareSchedule.effectiveWateringIntervalDaysForDisplay()` conversion the "Why this date?" sheet already uses before it's shown or compared against the current interval, so the dialog no longer even appears when the whole apparent change was just a unit mismatch. The editable field a user can retype before tapping Apply, and the Apply/commit write path itself, are unchanged — display/gating-only fix (#620)
