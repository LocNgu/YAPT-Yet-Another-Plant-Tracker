...
### Fixed
- **Reschedule watering now shows the resulting date beside Today and each +N-day option** — the +N choices still count from the current due date (or today if it has passed), while Custom date opens the date picker; no date-setting behavior changes (#737)
