...
### Added
- **Interval sliders now have −/+ stepper buttons for exact 1-unit nudges** — watering, fertilizing, and repotting on Add/Edit Plant, watering and fertilizing on Plant Detail's inline cards, and the dormant watering cadence slider all gained flanking decrease/increase buttons alongside the existing slider, plus a light haptic tick while dragging across a step. Buttons are disabled at each range's bounds, each has a setting-specific accessibility label, and the slider announces its current value after every change — including on Add/Edit Plant, which now also gets a live spoken value. On Plant Detail's watering/fertilizing cards, a burst of stepper taps now coalesces into a single save and a single history entry after a brief pause, rather than one for every tap, and that pending save is never lost even if you navigate away right after tapping (#531, product ADR-0048/product ADR-0050)
...
- **Fertilizing active seasons** — choose which of Spring, Summer, Autumn, and Winter a plant fertilizes in, at its one existing interval; an inactive season is never due or overdue, and re-entering an active season makes the plant due that day rather than "overdue by months." Seasons follow the device timezone's hemisphere and default to every season, so existing plants are unchanged. Add/Edit Plant and Plant Detail's Fertilize tab share the same inline, always-editable season chips (at least one must stay selected), `.yapt` backups preserve the choice, and a configured dormancy window independently pauses fertilizing reminders on top of this (see Changed below) (#286, #795, product ADR-0049, superseding product ADR-0045's four separate per-season intervals, which never shipped in a release)

### Changed
- **A plant's dormancy window now pauses fertilizing reminders as well as watering** — in 0.31.0, dormancy only suspended watering and fertilizing stayed on its normal schedule. A plant inside its dormancy window is now never shown as due or overdue for fertilizing: the Plant List's Fertilizing due sort groups it under Dormant, Calendar omits its fertilizing dates inside the window, and it drops out of the daily care notification's fertilizing reminders. The underlying fertilizing due date is still tracked and becomes active again once the window ends. Repotting and custom reminders are unaffected (#286, product ADR-0049)
...
- **Photos now take substantially less space in YAPT and, when requested, its backups** — new camera captures are resized to at most 1920 px on their longest side and re-encoded as 80% quality JPEGs, and existing in-app camera captures receive the same optimization once after updating. Backup export now offers an optional "Optimize photos" checkbox that compresses the copies placed in the backup—including gallery photos—without changing the originals in the system gallery; it starts unchecked so normal backups retain original fidelity. Orientation and PNG transparency are preserved, unsupported formats remain unchanged, and processing safely falls back to the original image if compression fails (#309, product ADR-0041)

### Fixed
- **A late "Soil was still moist" watering entered in Add Care Log can no longer shorten a plant's interval during its first or post-reset history bootstrap** — the form now passes that feedback to the shared bootstrap, which keeps the prior interval as a floor under product ADR-0033 (#780)
- **Reschedule watering now shows the resulting date beside each +N-day option** — the +N choices still count from the current due date (or today if it has passed), while Today uses the actual day tapped and Custom date opens the date picker; no date-setting behavior changes (#737)
- **Retyping a suggested watering interval now uses one consistent Apply-day clock** — after logging a watering for an earlier date, the dialog shows today's effective interval, and a retyped value is converted back to a seasonal base using the day Apply is tapped. The plant and adjustment timestamps use that same instant, rather than mixing a direct system-clock read with the injectable clock; reschedule's update timestamp now uses the injectable clock too. Accepting the unedited suggestion still preserves the model's precise base (#767)
- **The interval-suggestion dialog could fire purely because the season had moved on, and attribute it to whichever watering you'd just logged** — all three places that decide whether to show the dialog (Plant Detail, Calendar, Plant List) compared today's live seasonally-adjusted interval against `wateringIntervalDays`, a number only updated when you last edited the interval, applied a suggestion, or a couple of one-time migrations ran — so it silently drifted away from what your due date was actually showing between those events, worst around the spring/autumn equinoxes. Watering a day early could show "suggested +1 day" that had nothing to do with the watering at all. The comparison is now live on both sides: today's display value from before the watering versus today's display value from after it, so a suggestion only ever appears when the model itself actually moved something. "Currently N days" now reads the same live number on every screen. A follow-up fix corrected "today" itself: when logging a backdated watering (via Plant Detail's or Add Care Log's date pickers), both sides of that comparison were still being evaluated on the backdated date instead of the real day you're looking at the screen — which could either show a needless dialog for a number that wasn't actually going to change today, or, in the opposite and more concerning direction, silently apply a real change to today's display without ever asking, bypassing the "ask before changing intervals" setting entirely. Both sides now always compare against the actual current date (#716)
...
- **Applying an adaptive watering suggestion automatically (Settings → "Ask before changing intervals" off) still ratcheted the seasonal base** — `PlantDetailViewModel.applySuggestionOrPrompt()`'s silent-apply branch picked between two `QuickLogUseCase.applyWateringIntervalSuggestion()` call shapes based on whether the model's precise base happened to equal its own rounded value, and the no-base shape re-derives the base as `deseasonalize(effectiveInterval)`. Since `effectiveInterval` is already `round(base × season)`, that round-trip threw away the exact value the caller already had — a base of 7.0 at a 1.2 seasonal factor was stored back as 6.67 — and the residual fed the next observation, which is precisely the ratchet #718 set out to remove. Behaviour was discontinuous at the integer boundary too: 7.0 took the lossy path, 7.0001 the exact one. The precise base is now passed through unconditionally. Only ever reachable with the toggle off — the product ADR-0006 dialog path was already correct, which is why #718's own fix did not catch it (#718)
- **Reschedule's "Today" option could be greyed out in a state where it would actually pull the due date in** — it was gated on `PlantCareStatus.isOverdue`, which reflects the *effective* (post-override) due date; since a reschedule override only ever wins when it's later than the schedule-computed date, a plant rescheduled out to a future date could pass its schedule-computed due date and still show "Today" disabled, even though tapping it would legitimately shorten the earlier deferral. The gate now enables "Today" whenever tapping it would actually move the due date — comparing local today against both the schedule-computed due date and the plant's current effective due date, so a second tap after the date has already been pulled to today stays correctly disabled; `isOverdue`'s own definition is unchanged. A never-watered plant with a winning future override is a known, accepted, unfixed edge case — its computed due date always tracks "now", so this gate never enables there either, matching the old behavior (#746)
...
since the fixes and the removal are both real history, just no longer separately user-visible.
- **A second same-day "Soil still moist" reschedule silently dropped the new due date** — `QuickLogUseCase.recordStillMoistCheck()`'s same-day `CareType.CHECK` duplicate guard returned early before ever writing `Plant.wateringDueDateOverride`, so re-checking a plant later the same day and picking a different date changed nothing. The duplicate branch now still commits the picked date, skipping only the CHECK log/adaptive observation/adjustment row it would otherwise duplicate; the "already checked today" message now says the date still moved. It writes the override through a new column-specific `PlantDao.updateWateringDueDateOverride()` query rather than a full-row update, so two overlapping notification deliveries can't have one revert the other's confidence write. Affects both the Plant Detail reschedule prompt and the notification's Still-moist action (#714)
...
- **Rescheduling watering to a custom date before the schedule's own due date silently did nothing** — `CareSchedule.computeWateringDue()` resolves the due date as `maxOf(computedNextDueAt, override)`, so an override earlier than the schedule's own date was written to the database and then discarded, with no chip, snackbar, or error to say so. The "Custom date…" picker now also rejects any date on or before the schedule-computed due date (in addition to the existing before-today floor), so such a date is no longer offerable in the first place. A new `PlantCareStatus.computedNextWateringDueAt` carries the pre-override due date out of `CareSchedule` for the picker to compare against (#720)

## [0.30.0] - 2026-09-14

### Changed
- **The "Log watering" date picker now slides up as a bottom sheet instead of popping up as a centered dialog** — `LogWateringDatePickerDialog` swaps its `DatePickerDialog` container for a `ModalBottomSheet` (always fully expanded) wrapping the same Material3 `DatePicker` and OK/Cancel buttons, matching the bottom-sheet style already used elsewhere on Plant Detail (the watering/reschedule reason prompts, "Why this date?"). No behavior change — same not-future-only range, same pre-selected today, same wall-clock time-of-day on the logged entry (#675)
- **The Repot and Photo tab quick actions now ask for a date before logging, instead of logging immediately** — Repot opens the same bottom-sheet date picker the Water/Fertilize tabs use (defaulting to today), and confirming anchors both the `CareLog` and the existing repot confidence-reset/freeze window to the picked date; cancelling logs and resets nothing. Photo now opens one sheet with an editable date row plus **Take photo** / **Choose from gallery**, and writes its `CareLog` directly (updating the cover photo) rather than navigating to the generic Add Care Log screen — the trade-off is no notes field on this quick path, with the full `AddCareLogScreen` flow unchanged as the canonical way to add a photo with notes. Cancelling the sheet or abandoning image selection logs nothing (#694)
...
- **Dismissing the adaptive-interval suggestion dialog from Calendar or Plant List no longer silently changes confidence with nothing to show for it** — both screens raised `Plant.wateringConfidence` on dismissal but, unlike Plant Detail, never wrote the matching `DIALOG_DISMISSAL` row, so the change never appeared in the "Why this date?" sheet's "Recent adjustments" list. The confidence bump and row write are now a single shared `QuickLogUseCase.recordWateringSuggestionDismissal()` function all three screens call, so this class of surface-specific omission can't recur (#674)
...
### Fixed
- **Three backdated-quick-water edge cases in the #654 "Log watering" date picker, all only reachable when backdating to before an already-existing later watering** — flagged by an automated review of the 0.28.0 release PR. (1) `QuickLogUseCase.quickWaterWithReason`/`quickLiquidFertilizeWithReason` cleared an active reschedule (`Plant.wateringDueDateOverride`) on every WATER insert unconditionally; backfilling an old, forgotten watering from before that reschedule was made silently discarded it even though the backfilled entry has no bearing on the current due date — the override now only clears when the inserted `loggedAt` is at or after the plant's previous newest watering (`CareLogRepository.getLastTwoWaterings`, queried before the insert). (2) `PlantDetailScreen`'s on/off-schedule gate for the "Log watering" picker (and the subsequent `WateringReasonBottomSheet`'s gap-length wording) compared the chosen date against the plant's globally newest watering (`PlantCareStatus.lastWateredAt`, always "now"-relative) instead of that date's own chronological predecessor — a new `PlantDetailViewModel.previousWateringBefore()` suspend wrapper around `CareLogRepository.getLastWateringBefore()` (the same lookup `QuickLogUseCase.computeSuggestion()` already used) now feeds both. (3) A backdated quick-water triggering the #571/#662 cold-start history bootstrap (`WateringLifecycleReset.maybeBootstrap()`) converted the bootstrapped base interval to effective/seasonal space using the historical (backdated) day instead of today — reintroducing the exact staleness bug #662 fixed for the non-backdated path. `maybeBootstrap()` gained a separate, always-real-wall-clock `displayNow` parameter used only for that conversion; `WateringAdjustment.triggeredAt`/`Plant.updatedAt` are unchanged, still the historical timestamp (#679)
...
- **The confidence-weighted adaptive watering model is now always on** — `FeatureFlagRegistry.ADAPTIVE_WATERING` (`adaptive_watering`) has graduated out of developer mode: `CareSchedule.computeAdaptiveInterval()`'s multiplicative + confidence-weighted correction (target = observed gap × multiplier, step size shrinking as `Plant.wateringConfidence` rises) is now the only watering-suggestion path for every plant, with no flag to toggle it off. The legacy fixed ±1-day nudge (`CareSchedule.computeSuggestedInterval()`, product ADR-0006) it had been gating is removed along with the flag check. No behavior change for anyone who already had the flag on, with one narrow exception: `AddCareLogViewModel`'s quick-log suggestion no longer falls back to the legacy nudge for a plant with no configured `wateringIntervalDays` — it now returns no suggestion instead, aligning with `QuickLogUseCase`'s pre-existing identical guard. Plants that never had the flag enabled now get suggestions from the confidence-weighted model going forward. No DB migration — `Plant.wateringConfidence` and the `.yapt` backup field already shipped unconditionally regardless of the flag's state (#655)
...
### Fixed
- **The late-watering "Why was it late?" prompt no longer offers a confusing, mis-shortening "It was dry by then" answer** — that option read as accusatory, and its `TOO_LATE` mapping usually *lengthened* the interval rather than shortening it for a genuinely late gap, extrapolating a precise point estimate from what's really an ambiguous multi-day-overdue window. It's replaced with "Soil was still moist" (`WateringReason.SOIL_STILL_MOIST` → `TOO_SOON`, lengthens), the same signal the Reschedule flow's "Soil still moist" answer already represents, for a user who checked informally rather than tapping Reschedule. A late watering can no longer shorten the interval at all — shortening only ever happens from the early ("Why now?" → "The plant needed it") direction, where the signal is precise; this also closes a gap the #571 cold-start history bootstrap had, since its plain median-of-history estimate was blind to today's reason and could otherwise still shorten the interval on a plant's first adaptive observation. See product ADR-0033 (#649)
- **A liquid-fertilizer plant's combined Water+Fertilize action is now reachable from the Water tab, not only the Fertilize tab** — on the per-action tabs layout (`plant_detail_tabs`), the Water tab now shows a second, visually distinct "Water + Fertilize" `OutlinedButton` (`CombinedWaterFertilizeActionRow`) directly below the existing "Water" button whenever `Plant.useLiquidFertilizer` is true, wired to the same `requestLiquidFertilize()` path (the paired WATER+FERTILIZE quick-log, with the same off-schedule reason prompt) that `FertilizeDueActionRow` already uses — no tab switch needed to log the one action a liquid-fertilizer plant owner takes every time they water. The existing plain "Water" button is unchanged and always calls plain `requestWater()`, regardless of `useLiquidFertilizer` — an initial version of this fix instead made that same button silently branch its behavior underneath an unchanged "Water" label, which was corrected to this visibly distinct second button. For consistency, the Fertilize tab's button (`FertilizeDueActionRow`) is now also relabeled "Water + Fertilize" (sharing the same new string resource) when `useLiquidFertilizer` is true — its `onClick` behavior was already correct and is unchanged. Non-liquid-fertilizer plants and the classic (non-tabs) layout are unaffected (#652)
- **`WhatsNewContent.kt` now has the same two-tier `unreleased`/`all` shape `CHANGELOG.md` already has** — `WhatsNewContentTest.topEntryMatchesCurrentVersionName` pinned `all.first().versionName` to the currently-released `BuildConfig.VERSION_NAME` at all times, which made following the documented per-PR workflow (appending an entry for unreleased work) always fail CI. Implementers now append to a new `WhatsNewContent.unreleased: ReleaseNotes` per PR (mirroring `CHANGELOG.md`'s `[Unreleased]`); `WhatsNewSheet` keeps reading only `all`, so nothing unreleased is ever shown to a user. Release-cut promotes `unreleased` into `all` and resets it, per the documented release-cut checklist in `release.md` (no automated check, mirroring `CHANGELOG.md`'s own `[Unreleased]` promotion). Internal tooling/process fix only — no user-visible behaviour change (#601, product ADR-0032, supersedes ADR-0010's stated mechanism)
...
### Fixed
- **Plant Detail's seasonal watering curve no longer crashes the app when two adjacent y-axis ticks round to the same whole-day value** — the day-labeled axis (shown whenever `plant.wateringBaseIntervalDays` is set) used to blank a duplicate tick's label to `""` from the `CartesianValueFormatter`, which Vico 2.5.2's `VerticalAxis` treats as a fatal programming error (uncaught `IllegalStateException`, crashed the whole app on chart composition). Per Vico's own guidance, "which ticks get labeled" is now controlled by a custom `VerticalAxis.ItemPlacer` (`DayLabelItemPlacer`) instead — the formatter always returns a non-empty `"Nd"` string, and the item placer withholds the duplicate tick's *value* from `getLabelValues`/`getWidthMeasurementLabelValues`/`getHeightMeasurementLabelValues` so Vico is never asked to format it. All 5 fixed gridlines still render; only which ones get a text label changes, so the "don't repeat an adjacent duplicate day label" polish from #622 is fully preserved. The Settings screen's raw-multiplier axis is unaffected (#638)
...
- **The Calendar and Plant List "Water every N days?" suggestion dialogs had their own independent copies of the apply-suggestion write path, reproducing both the #626 drift bug and the older #572/ADR-0028 missing `wateringBaseIntervalDays` dual-write** — `CalendarViewModel.applySuggestedInterval()` and `PlantListViewModel.applySuggestedIntervalFromList()` each wrote the raw base-space suggestion straight into the literal `wateringIntervalDays` and never touched `wateringBaseIntervalDays` at all, neither going through the fix #626 already applied to Plant Detail. The write path is now a single `QuickLogUseCase.applyWateringIntervalSuggestion()` function — extracted from `PlantDetailViewModel.applyIntervalInternal()`, which now delegates to it — shared by all three screens, so this class of bug can no longer recur independently on any one of them (#631)
- **Seasonal watering curve preview chart's month labels no longer crop or overlap on narrow screens** — `SeasonalWateringCurveChart` (Settings amplitude picker and Plant Detail's Water tab inline-settings card) measures its actual plot width at runtime (`BoxWithConstraints`) instead of assuming there's always room for 12 full "Jan"–"Dec" labels at a fixed size. A new pure `resolveMonthLabelStrategy()` continuously degrades through a fallback order — full "MMM" labels at normal size, then shrunk toward a 9sp floor, then single-letter labels (J F M A M J J A S O N D) re-run through the same shrink-to-floor check, and only as a last resort alternating months left unlabeled (still ticked) — stopping at the first stage that fits without cropping/overlap. The underlying curve math (`SeasonalWateringCurveSampler`/`SeasonalWatering.season()`) is untouched; both render sites are fixed automatically since they share the one composable (#621). **Follow-up:** the chart was still cut off around November on real devices even after the above fix, because a separate, deeper issue — not a label-sizing one — was masked by never having been visually checked on-device: `rememberVicoZoomState(zoomEnabled = false)`'s default `initialZoom` is `Zoom.max(Zoom.fixed(), Zoom.Content)`, which floors the chart's real content width at its un-zoomed base (~384dp, from Vico's default 32dp-per-month point spacing with no configured `pointProvider`) regardless of how narrow the container actually is. On any card narrower than that, the excess silently overflowed past the right edge — where Nov/Dec sit — and was hard-clipped, since scrolling is intentionally disabled on this always-fully-visible chart. Passing `initialZoom = Zoom.Content` (no floor) makes the chart always scale to exactly fit its measured width; `resolveMonthLabelStrategy()`'s own slot-width model needed no changes, since it was already conservative relative to Vico's real per-tick geometry once that geometry actually matches the container
