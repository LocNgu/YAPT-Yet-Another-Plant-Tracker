...
### Fixed
- **A liquid-fertilizer plant's combined Water+Fertilize action is now reachable from the Water tab, not only the Fertilize tab** — on the per-action tabs layout (`plant_detail_tabs`), the Water tab's "Water" button now branches on `Plant.useLiquidFertilizer` the same way `FertilizeDueActionRow` already does, calling `requestLiquidFertilize()` (the paired WATER+FERTILIZE quick-log, with the same off-schedule reason prompt) instead of plain `requestWater()` when true — no tab switch needed to log the one action a liquid-fertilizer plant owner takes every time they water. Button copy/icon are unchanged (still plain "Water"); non-liquid-fertilizer plants and the classic (non-tabs) layout are unaffected (#652)
