...
`.Codex/skills/run-yapt/driver.sh`, which wraps `gradlew`, `adb`, and
...
```bash
.Codex/skills/run-yapt/driver.sh build              # ./gradlew assembleDebug
.Codex/skills/run-yapt/driver.sh launch              # ensure emulator + install if missing + am start
.Codex/skills/run-yapt/driver.sh screenshot <name>   # -> /tmp/yapt-shots/<name>.png (prints path)
.Codex/skills/run-yapt/driver.sh tap-text "Aloe Vera 2"   # uiautomator dump, tap first match's center
.Codex/skills/run-yapt/driver.sh tap <x> <y>         # raw coordinate tap (from a screenshot you inspected)
.Codex/skills/run-yapt/driver.sh text "hello"        # types text into a focused field
.Codex/skills/run-yapt/driver.sh back                # KEYCODE_BACK — note: from the app's root screen this exits to the launcher, not a sub-screen
.Codex/skills/run-yapt/driver.sh dump                # pulls uiautomator XML to /tmp/yapt-window-dump.xml, prints path
.Codex/skills/run-yapt/driver.sh logcat              # dumps current logcat filtered to the app's pid
.Codex/skills/run-yapt/driver.sh stop                # am force-stop
.Codex/skills/run-yapt/driver.sh test                # ./gradlew testDebugUnitTest
```

Typical flow for verifying a UI change:

```bash
.Codex/skills/run-yapt/driver.sh build
.Codex/skills/run-yapt/driver.sh launch
.Codex/skills/run-yapt/driver.sh screenshot before
# ... inspect the screenshot, then drive the flow you care about ...
.Codex/skills/run-yapt/driver.sh tap-text "Add Plant"
.Codex/skills/run-yapt/driver.sh screenshot after
```
...
- **The photo-reminder dialog can intercept your next tap.** Opening a
  ...
  `AlertDialog` (see AGENTS.md's Photo reminder feature) if that plant's
