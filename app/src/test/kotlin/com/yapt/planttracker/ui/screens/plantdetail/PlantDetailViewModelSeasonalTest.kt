 ...
 * threshold — mirrors `PlantDetailViewModelPlantIssueTest`'s precedent. Seasonal watering graduated
 * (#656) — amplitude defaults to [SeasonalAmplitude.STANDARD] unless a test explicitly stubs `OFF`.
 */
class PlantDetailViewModelSeasonalTest {
    ...
    private fun stubAmplitudeOff() {
        every { dataStore.data } returns flowOf(preferencesOf(SettingsKeys.SEASONAL_AMPLITUDE to "OFF"))
    }
    ...
    @Test
    fun `setWateringInterval with amplitude Off leaves wateringBaseIntervalDays untouched`() = runTest {
        stubAmplitudeOff()
    ...
    @Test
    fun `setWateringInterval de-seasonalizes the new value for a non-pinned plant`() = runTest {
        val monstera = plant()
        every { plantRepo.getPlantById(1L) } returns flowOf(monstera)
        coEvery { plantRepo.updatePlant(any()) } just runs
        val vm = makeVm()
    ...
    @Test
    fun `setWateringInterval on a pinned plant leaves base untouched`() = runTest {
    ...
    // previously here: amplitude on/unpinned updating wateringBaseIntervalDays, amplitude on writing
    // an effective-space wateringIntervalDays, amplitude off leaving wateringBaseIntervalDays
    // untouched) now lives in QuickLogUseCaseIntervalApplyTest, against
    ...
    @Test
    fun `undoSilentIntervalApply restores the captured base verbatim, never re-derived from beforeIntervalDays`() =
        runTest {
            // #626 regression: the old buggy formula
            // (`if (!p.pinIntervalToBase && amplitude != 0.0) beforeIntervalDays.toDouble() else
            // p.wateringBaseIntervalDays`) diverged from "restore verbatim" whenever amplitude wasn't Off
            // and the plant was unpinned — exactly this setup. beforeIntervalDays (7) and
    ...
    fun `pendingWateringSuggestion converts base-space suggestion to effective space`() =
        runTest {
            // #620 repro: suggestedWateringInterval (9) is base-space, while plant.wateringIntervalDays
    ...
    @Test
    fun `pendingWateringSuggestion's effective interval equals the raw suggestion when amplitude is Off`() =
        runTest {
            stubAmplitudeOff()
    ...
    fun `pendingWateringSuggestion is null when the effective suggestion equals current (unit-mismatch artifact)`() =
        runTest {
            ...
            // dialog must not appear at all in that case, not just show a "0 day" delta. Amplitude Off
            // makes the base->effective conversion an identity, so a raw suggestion equal to the literal
            // current interval is guaranteed to also be equal after conversion.
            stubAmplitudeOff()
            val monstera = plant().copy(wateringIntervalDays = 7)
            every { plantRepo.getPlantById(1L) } returns flowOf(monstera)
            val vm = makeVm()
