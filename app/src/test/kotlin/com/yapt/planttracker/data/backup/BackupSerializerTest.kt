...
class BackupSerializerTest {
    ...
    @Test
    fun `wateringResetAt and wateringFreezeUntil round-trip their stored values`() {
        val decoded = backupJson.decodeFromString(
            BackupRoot.serializer(),
            backupJson.encodeToString(BackupRoot.serializer(), fullRoot())
        )
        assertEquals(1_690_000_000_000L, decoded.plants[0].wateringResetAt)
        assertEquals(1_692_000_000_000L, decoded.plants[0].wateringFreezeUntil)
    }

    @Test
    fun `plant without wateringResetAt or wateringFreezeUntil defaults to null`() {
        val json = """
            {"schemaVersion":13,"exportedAt":1700000000000,"appVersion":"1.0",
             "plants":[{"id":1,"name":"Aloe","createdAt":1000000000000,"updatedAt":1100000000000}],
             "careLogs":[],
             "settings":{"notificationsEnabled":true,"reminderHour":9,"reminderMinute":0}}
        """.trimIndent()
    ...
    @Test
    fun `dormancyStartMonth and dormancyEndMonth round-trip their stored values`() {
        val decoded = backupJson.decodeFromString(
            BackupRoot.serializer(),
            backupJson.encodeToString(BackupRoot.serializer(), fullRoot())
        )
        assertEquals(11, decoded.plants[0].dormancyStartMonth)
        assertEquals(2, decoded.plants[0].dormancyEndMonth)
    }

    @Test
    fun `plant without dormancyStartMonth or dormancyEndMonth defaults to null`() {
        val json = """
            {"schemaVersion":16,"exportedAt":1700000000000,"appVersion":"1.0",
             "plants":[{"id":1,"name":"Aloe","createdAt":1000000000000,"updatedAt":1100000000000}],
             "careLogs":[],
             "settings":{"notificationsEnabled":true,"reminderHour":9,"reminderMinute":0}}
        """.trimIndent()
    ...
    @Test
    fun `archivedAt round-trips its stored value`() {
        val decoded = backupJson.decodeFromString(
            BackupRoot.serializer(),
            backupJson.encodeToString(BackupRoot.serializer(), fullRoot())
        )
        assertEquals(1_693_000_000_000L, decoded.plants[0].archivedAt)
    }

    @Test
    fun `plant without archivedAt defaults to null`() {
        val json = """
            {"schemaVersion":19,"exportedAt":1700000000000,"appVersion":"1.0",
             "plants":[{"id":1,"name":"Aloe","createdAt":1000000000000,"updatedAt":1100000000000}],
             "careLogs":[],
             "settings":{"notificationsEnabled":true,"reminderHour":9,"reminderMinute":0}}
        """.trimIndent()
